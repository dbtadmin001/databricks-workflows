...
def load_contract(layer: str) -> QualityContract:
    path = files("project_12_pos_inventory_analytics").joinpath("quality_contracts.json")
...
def build_profile(
    ...
    *,
    ...
    safe_sample_df = (
        dataframe.select(*sample_columns).orderBy("row_fingerprint").limit(contract.sample_size)
    )
    safe_sample = tuple(
        json.loads(row[0])
        for row in safe_sample_df.select(F.to_json(F.struct(F.col("*")))).collect()
    )
    failures = tuple(
        dict.fromkeys(
            (
...
def profile_and_record(
    ...
    *,
    ...
    persist_profile(
        spark,
        profile,
        f"{catalog}.{schema}.project_12_pos_inventory_analytics_stage_quality_profile",
    )
    return profile
